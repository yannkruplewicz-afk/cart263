...
function go_all_stuff() {
    ...
    window.addEventListener("click", function () {
        ...
        if (microphoneStarted === false) {
            ...
            setupMicrophone([soundRectangle, soundWave]);
        }
    })
    ...
    async function setupMicrophone(targetObjects) {
        ...
        try {
            ...
            function readMicrophoneLevel() {
                ...
                for (let i = 0; i < targetObjects.length; i++) {
                    targetObjects[i].micLevel = micLevel;
                }

                requestAnimationFrame(readMicrophoneLevel);
            }
        } catch (err) {
            console.log("error getting the microphone");
        }
    }
